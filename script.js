window.addEventListener('load', () => {
  try {
    if (typeof APP_CONFIG === 'undefined') {
      alert("ERRO GRAVE: O arquivo config.js não carregou! Verifique se você salvou o arquivo corretamente e se copiou o código inteiro.");
      return;
    }

    const c = APP_CONFIG;
    const fields = c.headers;
    const drop = new Set(Object.keys(c.dropdowns || {}));
    const formula = fields[c.formulaColumn ? c.formulaColumn - 1 : -1];
    const records = [];
    ...
    function toast(s){const t=$('toast');if(t){t.textContent=s;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),3000)}}
    ...
    function build(){
      ...
      if(typeof pcttDB !== 'undefined') {
        ...
        console.warn("Aviso: pctt_trf2.js não encontrado. O autopreenchimento de prazos não vai funcionar.");
      }
    }
    build();
    ...
    async function pdfCanvases(file){if(!window.pdfjsLib)throw new Error('PDF.js não carregou.');pdfjsLib.GlobalWorkerOptions.workerSrc='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';const pdf=await pdfjsLib.getDocument({data:await file.arrayBuffer()}).promise,a=[];for(let p=1;p<=pdf.numPages;p++){prog(5+(p/pdf.numPages)*15,`Abrindo página ${p} de ${pdf.numPages}...`);const page=await pdf.getPage(p),v=page.getViewport({scale:2.5}),x=document.createElement('canvas');x.width=v.width;x.height=v.height;await page.render({canvasContext:x.getContext('2d'),viewport:v}).promise;a.push(prepCanvas(x))}return a}
    ...
    async function process(file){
      if(!file) return;
      try {
        if(!window.Tesseract) throw new Error('Tesseract.js não carregou. Recarregue a página conectado à internet.');
        ...
        setTimeout(()=>$('progressBox').classList.add('hidden'),1500);
          ...
          // Limpa o botão para permitir selecionar a mesma foto novamente
    ...
    if(addBtn) addBtn.onclick=()=>{
        ...
        fields.forEach(n=>{const x=$('f_'+slug(n));if(x)x.value=''});
        const rb=$('rawBox'); if(rb) rb.classList.add('hidden');
